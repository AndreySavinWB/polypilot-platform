...
2. Прочитать `PolyPilot-Штаб/00_Старт/DOC_INDEX.md` — master-файлы и архив.
3. Если задача влияет на деньги, тарифы, paywall, упаковку ценности или CTA — прочитать `PolyPilot-Штаб/06_Монетизация/MONETIZATION_STATE.md`.
4. Проверить текущий этап и приоритет недели.
5. Проверить список того, что запрещено делать сейчас.
6. Сформулировать свою задачу в рамках текущего приоритета.

**Не создавать новый `.md`**, если тема уже есть в `DOC_INDEX.md`.
...
На текущем этапе главный bottleneck — **pipeline PIE → OPEN CARD v2**: live-события должны получать полный data contract (`crowdPulse`, PMA, `whaleCheck`, `checkedReview`, `aiOdds` из PIE).

Поэтому текущий приоритет:

```text
Этап 4c — sync_live_to_mvp.py + PIE fields → OPEN CARD v2 (все live)
```

Параллельно (CEO): Track A Funnel 1.0 · Pack #1 · Starter proof.
